...
前節3.2.1で作成した.hmrはジャンクションリードの完全一致を要求していないためにやや精度の低いリード
が含まれる。そこで3.2.1で作成した.hrファイルから、精度を保証した.hmrを作成するためにh2hを用いる
PpWT.fastq_i14h75r10_0.hr　という.hrファイルに対して

% h2h PpWT.fastq_i14h75r10_0.hr 

と実行することで、PpWT.fastq_i14h75r10_0.hr.hmr ができる
2カラム目を数値順にソートすると組み換え位置が順に並ぶので

% sort -k 2 -n PpWT.fastq_i14h75r10_0.hr.hmr > PpWT_CH.hmr
